...
/**
 * The class consumes messages off of the queue. Runnable to listen on port (on
 * its own thread)
 *
 * @author syntx
 */
public class QueueConsumer implements Runnable, Consumer {
	...
	private int MQ_SERVER_PORT; // default port(change from rabbitMq config file
								// 8albn fi /etc/rabbitMQ/config
	...
	private void loadConfig() throws IOException {
		...
		InputStream in = new FileInputStream("userapp/config/message_queues.properties");
		prop.load(in);
		in.close();
		MQ_SERVER_ADDRESS = prop.getProperty(this.getClass().getSimpleName()
				+ "_HOST");
		MQ_SERVER_PORT = Integer.parseInt(prop.getProperty(this.getClass()
				.getSimpleName() + "_PORT"));
		QUEUE_NAME = prop.getProperty(this.getClass().getSimpleName()
				+ "_QUEUE");
	}
	...
	/**
	 * Called when new message is available. ( a request is ack only if it can
	 * be executed).
	 */
	public void handleDelivery(String consumerTag, Envelope env,
		...
		try {
			Dispatcher.sharedInstance().dispatchRequest(
					new RequestHandle(Producer.class.getSimpleName()),
					serviceRequest);
		} catch (CannotAcceptRequestException e) {
			sendAck(deliveryTag, false);
			QueueConsumerListenerThread.sharedInstance().start(); // restarting
																	// the app
	...
	public ServiceRequest constructReq(Map<String, Object> request) {
		...
		Map<String, Object> requestParams = (Map<String, Object>) request
				.get(REQUEST_PARAMETERS_KEY);
